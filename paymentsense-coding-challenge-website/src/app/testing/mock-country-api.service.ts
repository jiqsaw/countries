...
import { ICountryParam } from '../models/country.model';
...
export class CountryApiService {
  ...
  public getCountries = jasmine.createSpy('getCountries').and.callFake(
    (params$: Observable<ICountryParam>) => {
      if (params$) {
        ...
        return params$.pipe(
          switchMap((params: ICountryParam) => {
            ...
            if (params.q) {
              returnData = returnData.filter(e => e.name.includes(params.q));
            }
            return of({
              meta: { total: 4 },
              data: returnData
